...
--  A esta tabla se la puede considerar como tabla de hechos porque contiene métricas cuantitativas, como por ejemplo CANTIDAD???
...
DELIMITER //
CREATE TRIGGER trg_AuditoriaActualizacionVariacionMensual
AFTER UPDATE ON estimacion_variacion_intermensual_provincia_tbo_cte
FOR EACH ROW
BEGIN
    INSERT INTO AuditoriaVariaciones (
        id_registro, tabla_afectada, accion, fecha_modificacion, usuario
    ) VALUES (
        NEW.id,
        'estimacion_variacion_intermensual_provincia_tbo_cte',
        'UPDATE',
        NOW(),
        USER()
    );
